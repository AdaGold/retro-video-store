...
# def check_in_rental():
#     form_data = request.get_json()
#     rental = db.session.query(Video, Customer, Rental).join(Video, Video.id == Rental.video_id)\
#         .join(Customer, Customer.id == Rental.customer_id).filter(Video.id == form_data["video_id"])
